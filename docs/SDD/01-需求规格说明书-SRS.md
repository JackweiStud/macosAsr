...
macosAsr 是一款 **macOS 菜单栏常驻** 的本地语音输入工具。用户配置全局快捷键后，在 **任意前台应用** 的光标位置进行 **边说边出字（live partial）** 的语音听写。
...
|------|------|
| 优先级 | P0 |
| 描述 | 用户可配置全局快捷键；**默认 PTT（按住说话，松键结束）** |
| 输入 | 键盘全局事件 |
| 输出 | 进入/退出听写 Session |
| 规则 | 需「输入监控」权限；**默认快捷键不得与系统/本机常用组合冲突**；冲突时设置页提示并引导重选 |

**验收**：

- 在备忘录中按住默认快捷键说话，松键后会话结束
...
| 设置项 | 默认值 |
|--------|--------|
| 快捷键 | **默认推荐 `Fn + V` 按住（PTT）**；首次启动可改（见 PRD §4.2.2） |
| 触发模式 | PTT |
| 语言 | Chinese |
...
| OQ-02 | Toggle 是否进 MVP | **P1 再做**；MVP 仅 PTT | 2026-05-21 |
| OQ-03 | 默认快捷键 | **`Fn + V` 按住（PTT）为推荐默认**；首次启动可改；Fn 系优先，fallback 见 PRD §4.2.2 | 2026-05-21 |
