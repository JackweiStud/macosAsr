...
macosAsr 是一款 **macOS 菜单栏常驻** 的本地语音输入工具。当前可直接试用的入口是 **全局 ⌥Z Toggle** 或菜单栏 **Start Live Dictation / Stop Live Dictation**，在 **任意前台应用** 的光标位置进行 **边说边出字（live partial）** 的语音听写。

**范围内**：

- 本地 Qwen3-ASR 语音识别（基于 ASR-QWEN 复制并精简的代码）
- 菜单栏 Start/Stop 触发
- 全局 **⌥Z** Toggle 触发（当前实现；`CGEventTap`）
- PTT / 可配置快捷键（后续版本，`Fn + V` 等为候选）
...
|------|------|
| 优先级 | P0 |
| 描述 | 当前版本支持 **菜单栏 Start/Stop** 与 **全局 ⌥Z Toggle**；后续版本可配置 **PTT（按住说话，松键结束）** 等模式 |
| 输入 | 键盘全局事件（`CGEventTap`）或菜单操作 |
| 输出 | 进入/退出听写 Session |
| 规则 | 全局 **⌥Z** 需「输入监控」+「辅助功能」；匹配后吞掉事件，避免文本框输入 `Ω`；菜单触发不依赖输入监控；后续 PTT 方案不得与系统/本机常用组合冲突 |

**验收**：

- 在备忘录中通过菜单栏 Start/Stop 或 **⌥Z** 能开始和结束会话
- **⌥Z** 在前台为其它 App 时无需打开菜单即可 Toggle
- 未授权输入监控时，有明确引导而非 silent fail
...
| 设置项 | 默认值 |
|--------|--------|
| 触发方式 | **⌥Z Toggle**（全局 + 菜单）；菜单 Start/Stop 仍可用；后续：PTT / 可配置 |
| 快捷键 | 当前固定 **⌥Z**；后续版本可配置；PTT 候选 `Fn + V` |
| 语言 | Chinese |
...
| OQ-02 | Toggle 是否进 MVP | **已实现**：当前为 **⌥Z Toggle**；菜单 Start/Stop 并存 | 2026-05-22 |
| OQ-03 | 当前默认入口 | **⌥Z Toggle**（全局）+ 菜单 Start/Stop；PTT 候选仍为 `Fn + V`（P1），见 PRD §4.2.2 | 2026-05-22 |
