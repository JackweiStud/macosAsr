...
**当前实现**：`MacApp/macosAsrApp/GlobalHotkeyMonitor.swift`

- Session 级 **`CGEventTap`**（`.cgSessionEventTap` + `.defaultTap`）
- 监听 **keyDown / keyUp**；匹配 **⌥Z**（`.maskAlternate` + keyCode `6` / `kVK_ANSI_Z`）时：
  - 主线程回调 `toggleLiveDictation()`
  - `return nil` 吞掉事件，防止前台 App 收到 **Ω**
- 需 **Input Monitoring** + **Accessibility**
- 独立线程 RunLoop 常驻；`tapDisabledByTimeout` 时自动 re-enable
- 启动时 `CGRequestListenEventAccess()`；未授权时弹窗引导

**后续（P1）**

- PTT：`flagsChanged` / `keyDown` + `keyUp` 配对
- 候选 **`Fn + V` 按住**；走 PRD §4.2.2 向导（检测系统 fn 设置冲突）
- Settings 可配置快捷键；持久化至 `config.json`
