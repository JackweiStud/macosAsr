...
final class LiveDictationController {
    private enum DictationRunState {
        case idle
        case dictating
        case stoppingAwaitingFinal
    }

    private let stateMachine: InjectionStateMachine
    private var runState: DictationRunState = .idle
    private var dropFinalAfterUserInput = false
    ...
    var isActive: Bool { runState == .dictating }
        ...
        DaemonManager.shared.startSession { [weak self] result in
            DispatchQueue.main.async {
                ...
                case .success:
                    ...
                    self.dropFinalAfterUserInput = false
                    self.runState = .dictating
    ...
    func stop() {
        guard runState == .dictating else { return }
        DaemonManager.shared.stopSession()
        runState = .stoppingAwaitingFinal
        AppLogger.log("live_dictation_stop")
    }

    func notifyUserInputInterrupted() {
        switch runState {
        case .dictating, .stoppingAwaitingFinal:
            stateMachine.onUserInputInterrupted()
            if runState == .stoppingAwaitingFinal {
                dropFinalAfterUserInput = true
                AppLogger.log("late_final_will_be_dropped_after_user_input")
            }
        case .idle:
            return
        }
    ...
    private func handle(_ event: DaemonEvent) {
        ...
        case "final":
            if let text = event.text, !text.isEmpty {
                if dropFinalAfterUserInput {
                    AppLogger.log("late_final_dropped_after_user_input len=\(text.count)")
                    break
                }
