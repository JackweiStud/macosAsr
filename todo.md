...
### [P1-3] partial 推理 O(n²)，长句拖垮"边说边出字" ✅已滚动提交优化 / 实机验收通过
...
- **最终修法（不动 mlx_audio）**：utterance 累计到约 8s 时滚动提交当前段 final，然后立即允许后续语音新建 utterance，恢复短窗 partial 刷新；8s 前不因短换气切句，到期若已有 0.40s 静音则优先在静音处切，否则硬切。
- **不采用**：滑动窗口 partial。它需要和已提交文本对账，容易重复、退格和重写，不适合作为当前系统最小修复。
- **验证**：连续说 10s、18s 以上长句，观察 live 文本持续增长，不再出现 8s 后连续卡顿；准确率未见明显下降。
- **执行记录**：新增 `partial_roll_min_seconds`、`partial_roll_silence_seconds` 和 `max_audio_window` / `roll_silence` final reason；更新测试覆盖“到期直接 final”“后续语音新建 utterance”“短静音滚动提交”。已通过 `python3 -m unittest discover -s tests`。
